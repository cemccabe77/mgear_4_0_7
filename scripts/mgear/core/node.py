...
def createBlendMatrixNode(mA, mB, target=False, transform='srt'):
    """Create Maya blend Matrix node.
    ...
    Returns:
        pyNode: Newly created blendMatrix node

    """
    node = pm.createNode("blendMatrix")
    for m, mi in zip([mA, mB], ['inputMatrix', 'target[0].targetMatrix']):
    ...
    if target:
        ...
        pm.connectAttr(node + ".outputMatrix", dm_node + ".inputMatrix")
        if 't' in transform:
