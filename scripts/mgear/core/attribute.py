...
def addTransformConnection(transformA, transformB, attr):
    """
    Create string attributes on both transforms and connects them
    """
    if pm.attributeQuery(attr, node=transformA, ex=True)==False:
        pm.addAttr(transformA, ln=attr, dt="string", k=False, hidden=True)
    
    if pm.attributeQuery(attr, node=transformB, ex=True)==False:
        pm.addAttr(transformB, ln=attr, dt="string", k=False, hidden=True)

    pm.connectAttr(transformA+"."+attr, transformB+"."+attr, f=True)

def getTransformConnection(transform, attr):
    """
    Get the transform, that the attr is connected to
    """
    connT = pm.listConnections(transform+"."+attr)[0]
    return connT  
