...
# Encore =================================
# ========================================
def getUParam(pos, crvNode):
    '''
    Get closest point on curve
    
    pos = ([])  [x, y, z] position
    crv = (dag) Node of curve
    '''

    # # Pymel Node Way (fails)
    # pocNde = pm.createNode('nearestPointOnCurve', n='pocNode', ss=True)
    # pocNde.inPositionX.set(pos[0])
    # pocNde.inPositionY.set(pos[1])
    # pocNde.inPositionZ.set(pos[2])
    # crvShp = dag.getShapes(crvNode)[0]
    # pm.connectAttr(crvShp.worldSpace[0], pocNde.inputCurve)
    # param = pm.getAttr(pocNde.parameter)
    # pm.delete(pocNde)
    # return param

    point = om.MPoint(*pos)
    curveFn = om.MFnNurbsCurve(getDagPath(crvNode))
    paramUtill=om.MScriptUtil()
    paramPtr=paramUtill.asDoublePtr()
...
def ikSplineOnCrv(crvNde, count, suffix='jnt'):
    '''
    Creates joints along curve.
    Creates ikSpline from joints

    crvNde = (dag) Node of curve
    count  = (int) Number of joints to create
    suffix = (str) Suffix to give joints
    '''

    # IK chain with last joint hack fix
    jnts = createEvenAlongCrv(crvNode=crvNde, objType='joint', objName=str(crvNde), 
                                count=count, chain=1, suffix=suffix)
    last  = pm.duplicate(jnts[-1], n=str(crvNde)+'_{}_twistHelperHack'.format(len(jnts)))[0]
    pm.parent(last, jnts[-1], r=0)
    pm.joint(jnts[-1], zso=1, ch=1, e=1, oj='xyz', sao='yup')
    # One more joint to orient the last joint hack fix
    lastOri = pm.duplicate(last, n='lastOrient')
    pm.parent(lastOri, last)
    pm.joint(last, zso=1, ch=1, e=1, oj='xyz', sao='yup')
    pm.delete(lastOri)
    pm.setAttr(last+'.tx', pm.getAttr(jnts[-1]+'.tx')/4)
    jnts.append(last)
    ikHdl = pm.ikHandle(solver='ikSplineSolver', startJoint=jnts[0], endEffector=last, 
            rootTwistMode=0, n=crvNde+'_ikSpline', 
            createCurve=0, curve=crvNde, simplifyCurve=False, 
            rootOnCurve=True, parentCurve=False)

    return jnts, ikHdl


def createEvenAlongCrv(crvNode, objType, objName, count, chain=0, jntAxis='xyz', 
                        keepCrv=1, suffix='jnt', radius=0.1, lra=True):
    '''
    ...
    crvNode = (dag) Node of curve
    keepCrv = (bol) Delete or keep the curve
    suffix  = (str) Suffix for objects created
    ...
    objList = []
    crvShp  = dag.getShapes(crvNode)[0]
    crvFn   = om.MFnNurbsCurve(getDagPath(crvNode))
    ...
    if count == 1:
        ...
        if objType == 'joint':
            num = 0
            jt = pm.createNode('joint', n=objName+str(num)+'_'+suffix)
            pm.xform(jt,t=[point.x,point.y,point.z])
            objList.append(jt)
        if objType == 'locator':
            num = 0
            item = pm.spaceLocator(n=objName+str(num)+'_'+suffix)
            
            # Used in strap.strapRigDorito()
            pm.addAttr(item, at='float', k=True, ci=True, sn='U', max=1.0, min=0.0)
            pm.addAttr(item, at='float', k=True, ci=True, sn='V', max=1.0, min=0.0)
            
            itemTra = pm.listRelatives(item, p=1)
            pm.xform(itemTra,t=[point.x,point.y,point.z])
            objList.append(item[0])

    else:
        if pm.getAttr(crvShp+'.form') == 2: # Detects if open or closed curve. 2=Periodic
            spacing = 1.0/(count)
        else:
            spacing = 1.0/(count-1)
    ...
    if len(objList)>0 and objType=='joint':
        # Orient joint
        for jt in objList:
            pm.joint(jt, edit=True, zso=True, sao='yup', oj=jntAxis)
            pm.setAttr(jt+'.radius', radius)
            if lra == True:
                pm.setAttr(jt+'.displayLocalAxis', 1)
