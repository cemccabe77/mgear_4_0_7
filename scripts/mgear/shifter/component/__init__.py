...
class Main(object):
    ...
    def addCtl(self,
        ...
        if bufferName in self.rig.guide.controllers.keys():
            ctl_ref = pm.duplicate(self.rig.guide.controllers[bufferName])[0]
            ctl = primitive.addTransform(parent, fullName, m)
            shapes = pm.listRelatives(ctl_ref, shapes=True)
            for shape in shapes:
                pm.parent(shape, ctl, relative=True, shape=True)
                pm.rename(shape, ctl + "Shape")
            pm.delete(ctl_ref)
        ...
        #markE
        ctlHierarchy = None
        if "ctlHierarchy" in self.settings or ctlHierarchyDict:
            if "ctlHierarchy" in self.settings:
                ctlHierarchy = self.settings["ctlHierarchy"]

                ctlHierarchyPickledString = None
                try:
                    ctlHierarchyPickledString = str(ctlHierarchy)
                except:
                    pass

                ctlHierarchy = pickle.loads(ctlHierarchyPickledString)
            else:
                ctlHierarchy = ctlHierarchyDict
            keys = ctlHierarchy.keys()
            for key in keys:
                suffix = ctlHierarchy[key]
                name = ctl.replace('ctl', suffix)
                grp = None
                if key == keys[0]:
                    grp = primitive.addTransform(parent, name, m)
                else:
                    parentSuffix = ctlHierarchy[key-1]
                    parent = ctl.replace('ctl', parentSuffix)
                    ...
                    if key == keys[-1]:
                        ...
                        pm.parent(ctl, name)
                if lp:
                    self.transform2Lock.append(grp)

        return ctl

    def pickleAndStoreCtlHierarchy(ctlHierarchy, guide_root):
        result = None
        try:
            ctlHierarchyPickledString = pickle.dumps(ctlHierarchy)
            result = cmds.setAttr('%s.ctlHierarchy' % guide_root, ctlHierarchyPickledString, type='string')
        except:
            pass

        return result

    def unpickleAndReturnCtlHierarchy(guide_root):
        result = None
        ctlHierarchyPickledString = None
        try:
            ctlHierarchyPickledString = str(cmds.getAttr('%s.ctlHierarchy' % guide_root))
        except:
            pass

        result = pickle.loads(ctlHierarchyPickledString)
    ...
    def addFullNameParam(self):
        ...
        if self.options["classicChannelNames"]:
            ...
            attr = self.addAnimEnumParam(
                name, "__________"+self.settings["comp_name"], 0, [name]) # Adding 'real' comp name (comp_name)
    ...
    def addDoritoConnections(self):
        """
        Create connections between a joint and an ik_cns node
        for 'dorito' type setup.
        """
        if self.jointList != []:
            if self.settings["connector"] == "dorito":
                for jnt in self.jointList:
                    cnsName ="_".join(jnt.split("_")[0:2])+"_ik_cns"
                    if pm.objExists(cnsName):
                        ikCns = pm.PyNode(cnsName)
                        attribute.addTransformConnection(jnt, 
                                                        ikCns, 
                                                        attr="inv_wm_conn_"+jnt)
