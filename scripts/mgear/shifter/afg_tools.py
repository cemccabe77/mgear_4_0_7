...
# encore
import os
from . import relative_guide_placement as relative_guide_placement
from . import relative_volume_guide_placement as relative_volume_guide_placement
...
# Encore
# =============================================================================
def autoFitBiped(*args):
    '''
    Auto fit the Encore biped template to geo obj named 'bodyMerged'
    '''
    script_dir = os.path.dirname(__file__)
    file = open(os.path.join(script_dir, 'auto_fit_templates/biped/mgear/bipedtemplateAssociate.json'))

    elements = json.loads(file.read())
    increment_value = 100 / len(elements['ordered_hierarchy'])
    starting_val = 0
    for x in relative_guide_placement.updateGuidePlacement(elements['ordered_hierarchy'],elements['relativeGuide_dict'],reset_scale=False):
        starting_val = starting_val + increment_value

def autoFitBipedVolume(*args):
    '''
    Auto fit the Encore biped volume guide template to geo obj named 'bodyMerged'
    '''
    script_dir = os.path.dirname(__file__)
    file = open(os.path.join(script_dir, 'auto_fit_templates/biped/volume/bipedvolumeAssociate.json'))

    elements = json.loads(file.read())
    increment_value = 100 / len(elements['ordered_hierarchy'])
    starting_val = 0
    for x in relative_volume_guide_placement.updateGuidePlacement(elements['ordered_hierarchy'],elements['relativeGuide_dict'],reset_scale=False):
        starting_val = starting_val + increment_value
