...
dt.Vector([0.0, 1.5, 0.0]), # root
dt.Vector([2.15197581888, 0.65054602411, 0.0]), # loc
dt.Vector([3.15197581888, 0.46777153206, 0.0]), # loc
dt.Vector([4.15197581888, 0.46777153206, 0.0]), # loc
dt.Vector([0.0, 0.0, 0.0]), # heel
dt.Vector([1.88809098645, 0.0, -0.849292524644]), # outpivot
dt.Vector([1.81035786173, 0.0, 0.818553363087]),  # inpivot
dt.Vector([4.24509872516, 0.0, 0.0])] #toepivot
...
class Component(component.Main):
    ...
    def addObjects(self):
        ...
        plane = [self.guide.apos[0], self.guide.apos[-1], self.guide.apos[-4]]
        ...
        # Tip ----------------------------------------------
        v = datatypes.Vector(self.guide.apos[-6].x,
                             self.guide.apos[-1].y,
                             self.guide.apos[-6].z)
        ...
        for i, pos in enumerate(bk_pos):
            ...
            if i == 0: # bk_ctl 1
                t = transform.getTransform(self.heel_ctl) # get position and rotation (and sheer)
                t = transform.setMatrixPosition(t, self.guide.apos[-1]) # replace position with new position, keep rotation from t.
    ...
    def addAttributes(self):
        ...
        self.angles_att[0].set(self.settings["angle0"])
        self.angles_att[1].set(self.settings["angle1"])
    ...
    def addConnection(self):
        ...
        self.connections["leg_2jntOffset_01"] = self.connect_leg_2jntOffset_01
    ...
    def connect_leg_2jntOffset_01(self):
        """Connector for leg 2jnt"""
        ...
        pm.connectAttr(self.parent_comp.blend_att, self.blend_att)
        pm.parent(self.root, self.parent_comp.ikoff_ctl)
        pm.parent(self.parent_comp.ik_ref, self.bk_ctl[-1])
        pm.parentConstraint(
            self.parent_comp.tws2_rot, self.fk_ref, maintainOffset=True)
