...
class Component(component.Main):
    ...
    def addObjects(self):
        ...
        for i, t in enumerate(transform.getChainTransform2(self.guide.apos,
            ...
            ik_cns = primitive.addTransform(
                self.root, self.getName("ik%s_cns" % i), t)

            ik_npo = primitive.addTransform(
                ik_cns, self.getName("ik%s_npo" % i), t)
    ...
    def addAttributes(self):
        # Ref
        self.ikref_lst = []
        for i,cns in enumerate(self.ik_cns):
    ...
    def connect_standardWithIkRef(self):
        ...
        for i in zip(self.ikref_lst, self.ik_cns):
            self.ikref_att = i[0]
            self.connectRef(self.settings["ikrefarray"], i[1])

        # self.connectRef(self.settings["ikrefarray"], self.ik_cns[0])
