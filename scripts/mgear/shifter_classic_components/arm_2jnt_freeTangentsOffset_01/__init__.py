...
class Component(component.Main):
    ...
    def addObjects(self):
        ...
        self.ikoff_ctl = self.addCtl(self.ik_ctl,
                                     "ikoff_ctl",
                                     m,
                                     self.color_ik,
                                     "cube",
        ...
        if self.settings["mirrorIK"]:
            if self.negate:
                ...
                self.ikoff_ctl.rz.set(self.ikoff_ctl.rz.get() * -1)
                self.ik_ctl.ry.set(self.ik_ctl.ry.get() * -1)
                self.ikoff_ctl.ry.set(self.ikoff_ctl.ry.get() * -1)
        else:
        ...
        self.ik_ctl_ref = primitive.addTransform(self.ikoff_ctl,
    ...
    def addAttributes(self):
        ...
        self.tangentVis_att = self.addAnimParam("Tangent_vis",
                                                ...
                                                "bool",
                                                False)
        self.ikOffsetVis_att = self.addAnimParam("ikOffset_vis",
                                                "IK Offset vis",
                                                "bool",
                                                False)
        self.ikCnsVis_att = self.addAnimParam("ikCns_vis",
                                                "IK Cns vis",
                                                "bool",
                                                False)
    ...
    def addOperators(self):
        ...
        if self.settings["ikTR"]:
            ...
            # Fix IK FK blending when rig is scaled (Encore)
            node.createBlendMatrixNode(self.fk2_ctl+".m", self.ik_ctl+".m", target=self.ikRot_npo, transform='s')
