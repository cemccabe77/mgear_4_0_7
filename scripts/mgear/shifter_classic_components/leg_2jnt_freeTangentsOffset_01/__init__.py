...
class Component(component.Main):
    ...
    def addObjects(self):
        ...
        self.ikoff_ctl = self.addCtl(
            ...
            d=self.size * .10,
            tp=self.ikcns_ctl)
        attribute.setKeyableAttributes(self.ikoff_ctl)
        attribute.setRotOrder(self.ikoff_ctl, "XZY")
        attribute.setInvertMirror(self.ikoff_ctl, ["tx", "ry", "rz"])
        ...
        self.upv_cns = primitive.addTransformFromPos(
            self.ikoff_ctl, self.getName("upv_cns"), v)
    ...
    def addAttributes(self):
        ...
        self.ikOffsetVis_att = self.addAnimParam(
            "ikOffset_vis", "IK Offset vis", "bool", False)
        self.ikCnsVis_att = self.addAnimParam(
            "ikCns_vis", "IK Cns vis", "bool", False)
    ...
    def addOperators(self):
        ...
        pm.parentConstraint(
            self.legChainUpvRef[0], self.ikoff_ctl, self.upv_cns, mo=True)
        ...
        for shp in self.root_ctl.getShapes():
            #connect the root control so it's hidden when in fk mode
        ...
        for shp in self.ikcns_ctl.getShapes():
            pm.connectAttr(self.ikCnsVis_att, shp.attr("visibility"))
        for shp in self.ik_ctl.getShapes():
            pm.connectAttr(self.blend_att, shp.attr("visibility"))
        for shp in self.line_ref.getShapes():
            pm.connectAttr(self.blend_att, shp.attr("visibility"))

        multNde = pm.createNode('multiplyDivide', n=self.getName("off_mult"), ss=True)
        pm.setAttr(multNde+'.operation', 1)
        pm.connectAttr(self.ikOffsetVis_att, multNde+'.input1X')
        pm.connectAttr(self.blend_att, multNde+'.input2X')
        for shp in self.ikoff_ctl.getShapes():
            pm.connectAttr(multNde+'.outputX', shp.attr("visibility"))
        ...
        # Fix IK FK blending when rig is scaled (Encore)
        ikfkNde = node.createBlendMatrixNode(self.fk2_ctl+".m", self.ikoff_ctl+".m", target=self.eff_loc, transform='s')
        ...
        # Fix shifter_classic_components: FreeTangent Limbs Round-Feature Issue #48
        if self.negate:
            plumin_nodeNeg1A = pm.createNode("plusMinusAverage")
            pm.connectAttr(muldiv_node.attr("outputX"), plumin_nodeNeg1A.attr("input1D[0]"))
            plumin_nodeNeg1A.attr("input1D[1]").set(180)
            plumin_nodeNeg1A.attr("operation").set(2)#subtract

            blend_nodeNeg1A = pm.createNode("blendColors")
            pm.connectAttr(plumin_nodeNeg1A.attr("output1D"), blend_nodeNeg1A.attr("color1R"))
            pm.connectAttr(self.roundness_att, blend_nodeNeg1A.attr("blender"))
            blend_nodeNeg1A.attr("color2").set(0,0,0)

            pm.connectAttr(blend_nodeNeg1A.attr("outputR"), self.tws1A_loc.attr("rz"))
        else:
        ...
        # Fix shifter_classic_components: FreeTangent Limbs Round-Feature Issue #48
        if self.negate:
            plumin_nodeNeg1B = pm.createNode("plusMinusAverage")
            pm.connectAttr(muldiv_node.attr("outputX"), plumin_nodeNeg1B.attr("input1D[0]"))
            plumin_nodeNeg1B.attr("input1D[1]").set(180)

            blend_nodeNeg1B = pm.createNode("blendColors")
            pm.connectAttr(plumin_nodeNeg1B.attr("output1D"), blend_nodeNeg1B.attr("color1R"))
            pm.connectAttr(self.roundness_att, blend_nodeNeg1B.attr("blender"))
            blend_nodeNeg1B.attr("color2").set(0,0,0)

            pm.connectAttr(blend_nodeNeg1B.attr("outputR"), self.tws1B_loc.attr("rz"))
        else:
            muldiv_nodeBias = pm.createNode("multiplyDivide")
            pm.connectAttr(muldiv_node.attr("outputX"),
                           muldiv_nodeBias.attr("input1X"))
    ...
    def setRelation(self):
        ...
        self.controlRelatives["ankle"] = self.ikoff_ctl
